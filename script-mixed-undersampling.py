...
print(SNR_db)
str_name_hdf5 = 'undersampling-mixed-db2-haar-sym2-arrays-snr=%d-db.hdf5' % SNR_db
print('*************************************************')
...
per_100 = 1
A_100, b_100 = UnderSample_mixed(r_c,riel_p,R_t,Nf,Np,per_100,Ski,f)
...
#        spgl1_mag_err, spgl1_phase_err,spgl1_psnr_vector =  GetErrorsSPGL1(x_c,y_c,r_c,riel_p,R_t,Nf,Np,per,Ski_noisy,wv,f,sigma,R_100, I_sim, cut_off, str_target)
        mag_err, phase_err,psnr_vector,spgl1_mag_err, spgl1_phase_err,spgl1_psnr_vector = GetErrorsMixedBoth(x_c,y_c,r_c,riel_p,R_t,Np,per,Ski_noisy,wv,f,sigma,R_100,I_sim,pers)
...
plt.savefig('us-mixed-target-%s-rmse-magnitude-omp-different-basis-with-errors-snr=%d.png' % (str_target,SNR_db),bbox_inches='tight')    
plt.close(fig)
fig, ax = plt.subplots(figsize=(12,6))
for i in range(len(wavelets)):
    ...
    ax.xaxis.set_tick_params(labelsize=fontsize)
    ax.yaxis.set_tick_params(labelsize=fontsize)
plt.savefig('us-mixed-target-%s-rmse-phase-omp-different-basis-with-errors-snr=%d.png' % (str_target.upper(),SNR_db),bbox_inches='tight')    
plt.close(fig)
fig, ax = plt.subplots(figsize=(12,6))
for i in range(len(wavelets)):
    #if i==2:
...
ax.legend(fontsize=fontsize)
plt.savefig('us-mixed-target-%s-psnr-phase-omp-different-basis-with-errors-snr=%d.png' % (str_target,SNR_db),bbox_inches='tight')    
plt.close(fig)
...
for i in range(len(wavelets)):
    ...
    ax.xaxis.set_tick_params(labelsize=fontsize)
    ax.yaxis.set_tick_params(labelsize=fontsize)
plt.savefig('us-mixed-target-%s-rmse-magnitude-spgl1-different-basis-with-errors-snr=%d.png' % (str_target,SNR_db),bbox_inches='tight')    
plt.close(fig)
...
for i in range(len(wavelets)):
    ...
    ax.xaxis.set_tick_params(labelsize=fontsize)
    ax.yaxis.set_tick_params(labelsize=fontsize)
plt.savefig('us-mixed-target-%s-rmse-phase-spgl1-different-basis-with-errors-snr=%d.png' % (str_target,SNR_db),bbox_inches='tight')    
plt.close(fig)
fig, ax = plt.subplots(figsize=(12,6))
for i in range(len(wavelets)):
    #if i==2:
    ...
    ax.xaxis.set_tick_params(labelsize=fontsize)
    ax.yaxis.set_tick_params(labelsize=fontsize)
plt.savefig('us-mixed-target-%s-psnr-phase-spgl1-different-basis-with-errors-snr=%d.png'%(str_target,SNR_db),bbox_inches='tight')    
plt.close(fig)
