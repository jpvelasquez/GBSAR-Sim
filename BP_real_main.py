...
def get_SAR_data():
    ...
    Sr_f = np.array(dset)#np.array(list(dset))
...
def plot_image(data2):
    ...
    fig.savefig(os.getcwd()+"/Plots/"+direction,orientation='landscape')

    return {'Im':Im, 'x_min':-Lx/2, 'x_max':Lx/2, 'y_min':yi, 'y_max':yi+Ly}

def main(dset_name):
    plt.close('all') # Cerrar todas las figuras previas
    print('Filename: ', dset_name)
    # Se declaran y cargan variables
    #f = hp.File(os.getcwd()+"/Data_set/"+dset_name,'r')
    #f = hp.File(os.getcwd()+"/Datos-reducidos/"+dset_name,'r')
    path = "Datos-reducidos/"+dset_name
    f = hp.File(path,'r')
    global dset, dinfo
    #dset = f['sar_dataset']
    #dinfo = f['info']
    dset = pd.read_hdf(path,'sar_dataset')
    dinfo = pd.read_hdf(path,'info')
    prm = sp.get_parameters2_reduced(dinfo)
...
if __name__ == '__main__':
    test = main("Datos-reducidos/dset_1.h5")
