...
def get_parameters():
    ...
    h = 10#10 #10 # 800 # 5 # Altura de la imagen(m)
...
def get_parameters2_reduced(dinfo):
    # Definición y lectura de parámetros
    c = 0.299792458 # Velocidad de la luz (x 1e9 m/s)
    #print(dinfo.head())
    fi = dinfo['fi'].values[0]*1e-9 # Frecuencia inferior(GHz)
    fs = dinfo['ff'].values[0]*1e-9 # Frecuencia superior(GHz)
    Nf = dinfo['nfre'].values[0] # Numero de frecuencias
    xi = float(dinfo['xi'].values[0]) # Posicion inicial(m)
    xf = float(dinfo['xf'].values[0]) # Posicion final(m)
    Np = dinfo['npos'].values[0] # Numero de posiciones
    dx = float(dinfo['dx'].values[0]) # Paso del riel(m)
    b_angle = dinfo['beam_angle'].values[0]/2 # Angulo del haz(°)
    time = np.array(dinfo['datetime'].values)[0]
    fc = (fi+fs)/2 # Frecuencia Central(GHz)
    BW = fs-fi # Ancho de banda(GHz)
    Ls = xf-xi # Longitud del riel (m)
    Ro = 0 # Constante debido al delay de los cables(m)
...
def get_parameters4(dset):
    ...
    # Definicion de las dimensiones de la imagen ("dh" y "dw" usados solo en BP)
    w = 400 # 20#Ancho de la imagen(m)
    h = 650 # 50#Altura de la imagen(m)
    hi = 300 # 50#Posicion inicial en el eje del rango
    dw = 1 #0.5# Paso en el eje del ancho(m)
    dh = 1 #0.5# Paso en el eje de la altura(m)
...
def get_matrix_data1():
    ...
    return I_t, rt, rt_x, rt_y, N_r, N_c
